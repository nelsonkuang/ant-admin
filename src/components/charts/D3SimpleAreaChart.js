...
class D3SimpleAreaChart extends React.Component {
    componentDidMount() {
        ...
        let chart = d3.select(this.chartRef).attr("width", width + margin.left + margin.right).attr("height", height2 + margin2.top + margin2.bottom);// 设置总宽高
        const rawData = this.props.rawData;
        const parseDate = d3.timeParse("%Y-%m");

        const data = rawData.map(item => { // 把原始数据转为d3可接受的数据
            return {...item, day:parseDate(item.day)}
        });

        let x = d3.scaleTime().range([0, width]).domain(d3.extent(data, function (d) { return d.day; })); // 设置mainChart x轴
        let x2 = d3.scaleTime().range([0, width]).domain(x.domain());  // 设置subChart x轴
        let y = d3.scaleLinear().rangeRound([height, 0]).domain([0, d3.max(data, function (d) { return d.quantity; })]);// 设置mainChart y轴
        let y2 = d3.scaleLinear().range([height2, 0]).domain(y.domain()); // 设置subChart y轴

        const brush = d3.brushX() // 设置brush
            .extent([[0, 0], [width, height2]])
            .on("brush end", brushed);

        const zoom = d3.zoom() // 设置zoom
        ...
        const mainAreaPath = d3.area() // 设置mainChart面积图路径
            ...
            .y1(function (d, i) { return y(d.quantity); })
            .y0(y(0));

        const xAxis = d3.axisBottom(x).tickFormat(d3.timeFormat("%Y年%m月"));

        let mainChart = chart.append("g").attr("class", "main-chart").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // 设mainChart在最外包层在总图上的相对位置

        mainChart.append("g") // 设置x轴
            .attr("class", "axis axis--x")
            .attr("transform", "translate(0," + height + ")")
            .call(xAxis);

        mainChart.append("g")// 设置y轴
        ...
        mainChart.selectAll('.axis--y .tick')// 画背景线
            .append('line')
            .attr('class', 'bg-line')
            .attr('stroke', '#fff1c9')
        ...
        let values = mainChart.append('g')// 输出图上的数值
            .attr('class', 'area--text');


        /***  subChart部分  ***/
        const subAreaPath = d3.area()// 设置subChart面积图路径
            .curve(d3.curveMonotoneX)
            .x(function (d) { return x2(d.day);})
            .y0(height2)
            .y1(function (d) { return y2(d.quantity);});
        let subChart = chart.append("g").attr("class", "sub-chart").attr("transform", "translate(" + margin2.left + "," + margin2.top + ")");// 设subChart在最外包层在总图上的相对位置

        subChart.append("g") // 设置x轴
            .attr("class", "axis axis--x")
            .attr("transform", "translate(0," + height2 + ")")
            .call(d3.axisBottom(x2).tickFormat(d3.timeFormat("%Y年%m月")));

        subChart.append('path')// 画面积图
            .attr("class", "area")
            .datum(data)
            .attr('d', subAreaPath)
            .attr('stroke', 'none')
        ...
        subChart.append("g")// 添加画刷
            .attr("class", "brush")
            .call(brush)
            .call(brush.move, [width * 2 / 3, width]);

        /***  其他部分  ***/
        chart.append("rect")// 添加刷放方块
            .attr("class", "zoom")
            .attr("width", width)
            .attr("height", height)
            .attr("cursor", 'move')
        ...
        function addPointsAndValues() { // 动态修改添加的点和值
