...
class D3SimpleVoronoiChart extends React.Component {
    componentDidMount() {
        ...
        let g = chart.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").on("touchmove mousemove", moved); // 设最外包层在总图上的相对位置

        let sites = d3.range(100) // 生成所有的点的坐标
            .map(function(d) { return [Math.random() * width, Math.random() * height]; });

        let voronoi = d3.voronoi() // 定义维诺图
            .extent([[-1, -1], [width + 1, height + 1]]);

        let z = d3.scaleOrdinal(d3.schemeCategory10);// 通用线条的颜色

        let polygon = g.append("g") // 画多边形
            .attr("class", "polygons")
          .selectAll("path")
        ...
        let link = g.append("g") // 画Delaunay三角网(三角形的每条边)
            .attr("class", "links")
          .selectAll("line")
        ...
        function redrawPolygon(polygon) {
          polygon
              ...
              .attr("fill", function(d, n) { return z(n); }) 
        }
        ...
        function redrawSite(site) {
          site
              .attr("r", function(d, i) { return i === 0 ? 4 : 2.5; })
